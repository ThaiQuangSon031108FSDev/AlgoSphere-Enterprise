...
public class DockerExecutionService : IExecutionService
{
    ...
    private static string BuildJsTracer(string userCode, string entryPoint, IEnumerable<TestCaseDto> testCases)
    {
        ...
        sb.AppendLine("const MAX_STEPS = 1000;");
        sb.AppendLine("const MAX_SIZE = 100;");
        sb.AppendLine();
        sb.AppendLine("function _checkOverflow() {");
        sb.AppendLine("  if (_step >= MAX_STEPS) {");
        sb.AppendLine("    throw new Error('TraceLimitExceededException: Execution exceeded the limit of 1000 visualizer steps.');");
        sb.AppendLine("  }");
        sb.AppendLine("}");
        sb.AppendLine();
        ...
        sb.AppendLine("    if (_nodeIdIdx >= MAX_SIZE) {");
        sb.AppendLine("      throw new Error('TraceLimitExceededException: ListNode count exceeded the limit of 100 elements.');");
        sb.AppendLine("    }");
        sb.AppendLine("    _checkOverflow();");
        ...
        sb.AppendLine("    if (_nodeIdIdx >= MAX_SIZE) {");
        sb.AppendLine("      throw new Error('TraceLimitExceededException: TreeNode count exceeded the limit of 100 elements.');");
        sb.AppendLine("    }");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'node', t: [this._id], v: { ..._v, val } });");
        sb.AppendLine("    return createNodeProxy(this, this._id);");
        ...
        sb.AppendLine("    },");
        sb.AppendLine("    set(target, prop, value) {");
        sb.AppendLine("      if (target.length >= MAX_SIZE && prop === 'length') {");
        sb.AppendLine("        throw new Error('TraceLimitExceededException: Array size exceeded the limit of 100 elements.');");
        sb.AppendLine("      }");
        ...
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine("class TracedStack {");
        sb.AppendLine("  constructor(name = 'stack') { this._name = name; this._items = []; }");
        sb.AppendLine("  push(val) {");
        sb.AppendLine("    if (this._items.length >= MAX_SIZE) {");
        sb.AppendLine("      throw new Error('TraceLimitExceededException: Stack size exceeded the limit of 100 elements.');");
        sb.AppendLine("    }");
        sb.AppendLine("    this._items.push(val);");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'push', t: [this._items.length - 1], v: { ..._v, val, name: this._name } });");
        sb.AppendLine("  }");
        sb.AppendLine("  pop() {");
        sb.AppendLine("    const val = this._items.pop();");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'pop', t: [this._items.length], v: { ..._v, val, name: this._name } });");
        sb.AppendLine("    return val;");
        sb.AppendLine("  }");
        sb.AppendLine("  peek() { return this._items[this._items.length - 1]; }");
        sb.AppendLine("  get length() { return this._items.length; }");
        sb.AppendLine("  isEmpty() { return this._items.length === 0; }");
        sb.AppendLine("}");
        sb.AppendLine("global.TracedStack = TracedStack;");
        sb.AppendLine();
        sb.AppendLine("class TracedQueue {");
        sb.AppendLine("  constructor(name = 'queue') { this._name = name; this._items = []; }");
        sb.AppendLine("  enqueue(val) {");
        sb.AppendLine("    if (this._items.length >= MAX_SIZE) {");
        sb.AppendLine("      throw new Error('TraceLimitExceededException: Queue size exceeded the limit of 100 elements.');");
        sb.AppendLine("    }");
        sb.AppendLine("    this._items.push(val);");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'enq', t: [this._items.length - 1], v: { ..._v, val, name: this._name } });");
        sb.AppendLine("  }");
        sb.AppendLine("  dequeue() {");
        sb.AppendLine("    const val = this._items.shift();");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'deq', t: [0], v: { ..._v, val, name: this._name } });");
        sb.AppendLine("    return val;");
        sb.AppendLine("  }");
        sb.AppendLine("  peek() { return this._items[0]; }");
        sb.AppendLine("  get length() { return this._items.length; }");
        sb.AppendLine("  isEmpty() { return this._items.length === 0; }");
        sb.AppendLine("}");
        sb.AppendLine("global.TracedQueue = TracedQueue;");
        sb.AppendLine();
        sb.AppendLine("class TracedMap extends Map {");
        sb.AppendLine("  set(k, v) {");
        sb.AppendLine("    if (this.size >= MAX_SIZE) {");
        sb.AppendLine("      throw new Error('TraceLimitExceededException: Map size exceeded the limit of 100 elements.');");
        sb.AppendLine("    }");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'hset', t: [], v: { ..._v, key: k, val: v, name: 'Map' } });");
        sb.AppendLine("    return super.set(k, v);");
        sb.AppendLine("  }");
        sb.AppendLine("  get(k) {");
        sb.AppendLine("    const val = super.get(k);");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'hget', t: [], v: { ..._v, key: k, val, name: 'Map', found: super.has(k) } });");
        sb.AppendLine("    return val;");
        sb.AppendLine("  }");
        sb.AppendLine("  has(k) {");
        sb.AppendLine("    const found = super.has(k);");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'hget', t: [], v: { ..._v, key: k, name: 'Map', found } });");
        sb.AppendLine("    return found;");
        sb.AppendLine("  }");
        sb.AppendLine("  delete(k) {");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'hdel', t: [], v: { ..._v, key: k, name: 'Map' } });");
        sb.AppendLine("    return super.delete(k);");
        sb.AppendLine("  }");
        sb.AppendLine("}");
        sb.AppendLine("global.Map = TracedMap;");
        sb.AppendLine();
        sb.AppendLine("class TracedSet extends Set {");
        sb.AppendLine("  add(v) {");
        sb.AppendLine("    if (this.size >= MAX_SIZE) {");
        sb.AppendLine("      throw new Error('TraceLimitExceededException: Set size exceeded the limit of 100 elements.');");
        sb.AppendLine("    }");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'vis', t: [], v: { ..._v, ds: { name: 'Set', op: 'add', val: v } } });");
        sb.AppendLine("    return super.add(v);");
        sb.AppendLine("  }");
        sb.AppendLine("  has(v) {");
        sb.AppendLine("    _checkOverflow();");
        sb.AppendLine("    _trace.push({ s: ++_step, a: 'vis', t: [], v: { ..._v, ds: { name: 'Set', op: 'has', val: v } } });");
        sb.AppendLine("    return super.has(v);");
        sb.AppendLine("  }");
        sb.AppendLine("}");
        sb.AppendLine("global.Set = TracedSet;");
        sb.AppendLine();
        sb.AppendLine("global._ptr = (name, pos, val) => {");
        sb.AppendLine("  _checkOverflow();");
        sb.AppendLine("  _trace.push({ s: ++_step, a: 'ptr', t: [pos], v: { ..._v, name, val } });");
        sb.AppendLine("};");
        sb.AppendLine("global._dpset = (row, col, val, formula) => {");
        sb.AppendLine("  _checkOverflow();");
        sb.AppendLine("  _trace.push({ s: ++_step, a: 'dpset', t: [row, col], v: { ..._v, val, formula } });");
        sb.AppendLine("};");
        sb.AppendLine("global._gedge = (u, v, w, dir = false) => {");
        sb.AppendLine("  _checkOverflow();");
        sb.AppendLine("  _trace.push({ s: ++_step, a: 'gedge', t: [u, v], v: { ..._v, weight: w, directed: dir } });");
        sb.AppendLine("};");
        sb.AppendLine("global._gvis = (nodeId, state = 'visiting') => {");
        sb.AppendLine("  _checkOverflow();");
        sb.AppendLine("  _trace.push({ s: ++_step, a: 'gvis', t: [nodeId], v: { ..._v, state } });");
        sb.AppendLine("};");
        sb.AppendLine("global._call = (depth, fnName, args) => {");
        sb.AppendLine("  _checkOverflow();");
        sb.AppendLine("  _trace.push({ s: ++_step, a: 'call', t: [depth], v: { ..._v, fn: fnName, args } });");
        sb.AppendLine("};");
        sb.AppendLine("global._ret = (depth, val) => {");
        sb.AppendLine("  _checkOverflow();");
        sb.AppendLine("  _trace.push({ s: ++_step, a: 'ret', t: [depth], v: { ..._v, val } });");
        sb.AppendLine("};");
        sb.AppendLine();
